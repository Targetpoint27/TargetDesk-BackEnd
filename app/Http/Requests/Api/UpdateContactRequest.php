...
class UpdateContactRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        $clientId = $contact ? $contact->client_id : null;
        ...
        return [
            ...
            // Emails (optionnels en update, mais si fournis, au moins un requis)
            'emails' => 'nullable|array|min:1',
            'emails.*.email' => [
                'required',
                'email',
                'max:255',
                function ($attribute, $value, $fail) use ($clientId, $contactId) {
                    if (!$clientId) return;

                    // Vérifier l'unicité de l'email dans le client (excluant le contact actuel)
    ...
    public function messages()
    {
        return [
            ...
            'emails.min' => 'Au moins un email est requis si des emails sont fournis',
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if (!empty($phones)) {
                ...
                // Si aucun téléphone n'est marqué comme principal, marquer le premier
